...
variable "cilium_loadbalancer_acceleration" {
  ...
  validation {
    condition     = contains(["native", "best-effort"], var.cilium_loadbalancer_acceleration)
    error_message = "cilium_loadbalancer_acceleration must be one of: native or best-effort"
  }
}
